...
-- Menu configuration for SmartPeek
local menu = {
...
local function extrapolate_position(player, ticks, x, y, z)
    local vx, vy, vz = entity.get_prop(player, "m_vecVelocity")
    if not vx then return x, y, z end -- Fallback if velocity is unavailable
    local interval = globals.tickinterval()
    return x + interval * vx * ticks, y + interval * vy * ticks, z + interval * vz * ticks
end

local function is_player_in_air(player)
    return bit.band(entity.get_prop(player, "m_fFlags") or 0, 1) == 0
end
...
local function get_end_position(origin, dest)
    local local_player = entity.get_local_player()
    if not client.trace_line then return dest, 1 end -- Fallback if trace_line is unavailable
...
local function run_all_points(debug_line, debug_circle, debug_fraction, depart_count, origin)
    ...
    for _, point_data in ipairs(points_list) do
        ...
        if debug_circle then
            draw_circle_3d(end_pos.x, end_pos.y, dz + 5, 5, r, g, b, a)
        end
...
local function get_peek_hitboxes(selected)
    ...
    if contains(selected, 'Chest') then
        hitboxes[#hitboxes + 1] = 4
        hitboxes[#hitboxes + 1] = 5
        hitboxes[#hitboxes + 1] = 6
    end
    if contains(selected, 'Stomach') then
        hitboxes[#hitboxes + 1] = 2
        hitboxes[#hitboxes + 1] = 3
    end
    if contains(selected, 'Arms') then
        hitboxes[#hitboxes + 1] = 13
        hitboxes[#hitboxes + 1] = 14
        hitboxes[#hitboxes + 1] = 15
        hitboxes[#hitboxes + 1] = 16
        hitboxes[#hitboxes + 1] = 17
        hitboxes[#hitboxes + 1] = 18
    end
    if contains(selected, 'Legs') then
        hitboxes[#hitboxes + 1] = 7
        hitboxes[#hitboxes + 1] = 8
        hitboxes[#hitboxes + 1] = 9
        hitboxes[#hitboxes + 1] = 10
    end
    if contains(selected, 'Feet') then
        hitboxes[#hitboxes + 1] = 11
...
local function smart_peek_runner()
    ...
    if ui.get(menu.target) == 'All Enemies' then
        ...
        for _, point in ipairs(points) do
            for _, player in ipairs(players) do
                for _, hitbox in ipairs(hitboxes) do
                    ...
                    if dmg >= math.min(ui.get(minimum_damage), entity.get_prop(player, 'm_iHealth') or 100) then
        ...
        for _, point in ipairs(points) do
            for _, hitbox in ipairs(hitboxes) do
                ...
                local e_vec = vector(new_x, new_y, new_z)
                local _, dmg = client.trace_bullet(me, point.x, point.y, point.z, e_vec.x, e_vec.y, e_vec.z)
                if dmg > math.min(ui.get(minimum_damage), entity.get_prop(best_target, 'm_iHealth') or 100) then
    ...
    if #targets > 0 then
        ...
        if contains(debug_options, 'Predict-Target Line') and x1 then
            ...
            if x2 then renderer.line(x1, y1, x2, y2, r, g, b, a) end
        end
        if x1 then
            renderer.text(x1, y1 - 12, r, g, b, a, nil, 0, tostring(math.floor(damage)))
        end
