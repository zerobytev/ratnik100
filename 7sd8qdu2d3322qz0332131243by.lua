...
local clantag_frames = { "b7", "b7t ", "b7t3", "b7t3.", "b7t3.t3", "b7t3.t3c", "b7t3.t3ch ", "byte.tech" }
local last_clantag_time = 0
local function set_clantag()
    local current_time = globals.realtime()
    if current_time - last_clantag_time >= 0.7 then
        local frame = math.floor(current_time * 1.5) % #clantag_frames + 1
...
local enable_resolver = ui.new_checkbox("RAGE", "Other", "Resolver (Beta!)")
local max_history = 18
local desync_threshold = 18
local jitter_threshold = 30
local max_desync_history = 14
local update_interval = 0.012
...
local function detect_anti_aim_type(records)
    if #records.last_angles < 5 then return "none" end
    ...
    local tick_deltas = {}
    for i = 2, #records.last_angles do
        local delta = math.abs(records.last_angles[i].angles[2] - records.last_angles[i-1].angles[2])
        yaw_deltas[i-1] = delta
        max_delta = math.max(max_delta, delta)
        sim_time_deltas[i-1] = records.last_angles[i].sim_time - records.last_angles[i-1].sim_time
        tick_deltas[i-1] = records.last_angles[i].tick - records.last_angles[i-1].tick
        if records.last_angles[i].hitbox_pos and records.last_angles[i-1].hitbox_pos then
            position_changes[i-1] = calculate_distance(records.last_angles[i].hitbox_pos, records.last_angles[i-1].hitbox_pos)
        else
    ...
    if is_teleporting then
        return "teleport"
    elseif max_delta > jitter_threshold then
        return "jitter"
    elseif avg_delta > desync_threshold then
        return "desync"
    elseif avg_sim_time < sim_time_anomaly_threshold or avg_tick_delta > 2 then
        return "fakelag"
    elseif max_delta > 75 then
        return "spinbot"
    elseif avg_delta > 10 and #records.shot_records > 2 then
        return "custom"
    end
    return "static"
end
...
local function predict_desync(player, steam_id, records, aa_type)
    if #records.last_angles < 3 then return records.learned_side end
    ...
    local side = records.learned_side
    if aa_type == "teleport" then
    ...
    elseif aa_type == "jitter" then
        ...
        table.insert(records.desync_history, side)
    elseif aa_type == "desync" then
        ...
        avg_delta = avg_delta / #yaw_deltas
        if avg_delta > desync_threshold or is_moving then
            side = yaw_deltas[#yaw_deltas] > 0 and 1 or -1
            table.insert(records.desync_history, side)
        end
    elseif aa_type == "spinbot" then
        side = records.desync_history[#records.desync_history] or (globals.tickcount() % 2 == 0 and 1 or -1)
    ...
    if #records.desync_history > max_desync_history then
        table.remove(records.desync_history, 1)
    end

    local side_count = 0
    for _, s in ipairs(records.desync_history) do
        side_count = side_count + s
    end
    return side_count >= 0 and 1 or -1
end
...
function resolver.record_player(player)
    ...
    local sim_time = entity.get_prop(player, "m_flSimulationTime")
    if sim_time == resolver.last_simulation_time[steam_id] then return end
    ...
    table.insert(records.last_angles, {
        angles = eye_angles,
        sim_time = sim_time,
    ...
    if #records.last_angles > max_history then
        table.remove(records.last_angles, 1)
    end

    records.aa_type = detect_anti_aim_type(records)
    resolver.last_simulation_time[steam_id] = sim_time

    if records.aa_type == "teleport" or records.aa_type == "fakelag" or records.aa_type == "custom" then
        records.anomaly_detected = true
    end
end
...
function resolver.resolve_angles(player)
    ...
    local base_angle = records.last_angles[#records.last_angles].angles[2]
    local resolve_angle = base_angle

    if records.learned_side then
        local desync_amount = aa_type == "spinbot" and 120 or aa_type == "teleport" and 140 or aa_type == "fakelag" and 110 or aa_type == "custom" and 100 or 60
        desync_amount = desync_amount * (1 + math.min(records.missed_shots * 0.4, 2.0))
        desync_amount = math.min(desync_amount, aa_type == "spinbot" and 180 or aa_type == "teleport" and 170 or 140)
        local smooth_factor = aa_type == "jitter" and 0.85 or aa_type == "teleport" and 0.95 or aa_type == "fakelag" and 0.9 or aa_type == "custom" and 0.8 or 0.7
...
function resolver.on_shot_fired(e)
    ...
    local records = resolver.player_records[steam_id]
    table.insert(records.shot_records, {
        tick = e.tick,
        predicted_angle = records.last_yaw or records.last_angles[#records.last_angles].angles[2],
        hit = e.hit,
        teleported = e.teleported,
        damage = e.damage
    })

    records.missed_shots = e.hit and 0 or records.missed_shots + 1

    if #records.shot_records > 16 then
        table.remove(records.shot_records, 1)
    end
